<template>
  <div class="app-page">
    ...
    <el-dialog v-model="detailVisible" title="资料详情" width="720px">
      <pre style="white-space:pre-wrap">{{ JSON.stringify(detail, null, 2) }}</pre>
    </el-dialog>
  </div>
</template>
...
import { apiAuditPending, apiAuditDo, type ProfileDTO } from '@/api/admin'
...
async function onAudit(row: ProfileDTO, result: 1 | 2) {
  let comment = ''
  if (result === 2) {
...
function onView(row: ProfileDTO) { detail.value = row; detailVisible.value = true }
onMounted(() => load(1))
</script>
